...
public class IntervalST<Value> {
    ...
    private Value get(Node x, Interval1D interval) {
        if (x == null)
            return null;
        int cmp = compareIntervals(interval, x.interval);
        if (cmp < 0)
    ...
    private Node remove(Node h, Interval1D interval) {
        if (h == null)
            return null;
        int cmp = compareIntervals(interval, h.interval);
        if (cmp < 0)
     ...
     ***************************************************************************/
    // 比較兩個 Interval1D，先比 min，再比 max
    private static int compareIntervals(Interval1D a, Interval1D b) {
        if (a.min() < b.min())
            return -1;
        if (a.min() > b.min())
            return 1;
