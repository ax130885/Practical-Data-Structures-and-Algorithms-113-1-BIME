...
3. Hibbard Deletion  
   ...
   ![Hibbard Deletion 程式實作](image-27.png)
...
2. 有向圖 Digraph  
   ...
   2. 強連通分量 strong components
      【概念】
      ![強連通分量 概念](image-49.png)
      類似一般的CC(連通分量)，但是在有向圖當中，可能每個點的連接，來回是不同路徑。  
      將圖分成強連通分量以後，可以找到以強連通分量為單位的DAG。  

      【Kosaraju-Sharir 概念】
      ![範例輸入](image-46.png) **範例輸入**  
      ![將所有邊都反向以後，做一次dfs](image-50.png) **將所有邊都反向以後，做一次dfs**  
      ![按照剛剛結果的順序，在原本沒有反向的圖上，再做一次dfs](image-51.png) **按照剛剛結果的順序，在原本沒有反向的圖上，再做一次dfs**  
      正反向各做一次，相當於確保CC內的每個邊，從兩個方向都有路徑可走。

      【Kosaraju-Sharir 實作】
      ![連通分量(無向) 實作](image-47.png) **連通分量(無向) 實作**
      ![強連通分量(有向) 實作](image-48.png) **強連通分量(有向) 實作**
...
6. 貪心 MST 算法:
   ...
   2. Prim   
      ...
      ![Lazy Prim 概念1](image-18.png)
      ![Lazy Prim 概念2](image-19.png)
      **Lazy Prim 概念**
      ...
      - 【Eager Prim 積極算法說明】把vertex放到優先佇列  
         ...
         ![Eager Prim 概念1](image-21.png)  
         ![Eager Prim 概念2](image-22.png)
      ...
      - 【最小優先級佇列 實作】
         ...
         ![最小優先級佇列 API](image-23.png) **最小優先級佇列 API**  
         ![最小優先級佇列 概念](image-24.png) **最小優先級佇列 概念 (紅字是qp[i]) (與上面 eager prim 的圖例沒有關係)**  
...
# 最短路徑(有權重的有向圖)
## 權重有向圖的建立
   ![權重有向邊的實作](image-30.png) **權重有向邊的實作**  
   類似先前(圖-5.權重邊圖)的實作，只差在either, other改成from, to(因為有方向性)。

   ![權重有向圖的API](image-31.png)
   
   ![權重有向圖的鄰接表實作](image-32.png)

## 通用性質
   ### relax 鬆弛
   ![relax 程式實作](image-33.png)
   ...
   - iff順向證明
      ![iff順向證明](image-34.png) 假設最短路徑上的每一條邊都滿足公式，把每一條邊的結果都相加以後，還是會滿足公式(中間的邊可以消掉)。
...
### Dijkstra (BFS)(權重不能有負)
![Dijkstra 實作](image-35.png)
![Dijkstra 實作](image-36.png)
Dijkstra 實作
![Dijkstra 複雜度](image-37.png)
...
![Bellman Ford 概念](image-44.png) **Bellman Ford 概念**  
![Bellman Ford 程式實作](image-45.png) 一開始先初始化，然後就開始重複v-1次把所有邊都relax，如果做到某次relax都不更新，可以提早終止。  
...
## 三種最短路徑算法 複雜度比較
![三種最短路徑算法 複雜度比較](image-43.png) **三種最短路徑算法 複雜度比較**

## 最短路徑的應用 Content-aware resizing 裁縫拼接
![Content-aware resizing 概念](image-39.png) 維持城堡與人不變形，只縮放不重要的背景部分，並且縮放結果看起來也很合理。  
概念上，算法會找出一系列的接縫，接著利用接縫對圖像做縮放。
1. 首先，我們拿到一張需要縮小的照片
2. 計算照片中每一個像素的灰階強度
3. 找到圖中數條強度較低的seams(接縫)
   ![seams方法說明](image-42.png)
   - 每個像素連接到下方三個鄰居
   - weight of pixel設定為影像強度的差異
   - 找到最短路徑
   - 這個最短路徑的元素最接近，代表移除或膨脹對整體效果最不明顯
4. ![Seams 在gradient magnitude圖中的結果](image-40.png) Seams 在gradient magnitude圖中的結果
